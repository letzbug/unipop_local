(async function(){
 ...
 async function doPrint(){if(assignment.showPrint===false||!current)return;const a=UniStore.canPrint(screenId),t=$('toast');if(!a.ok){t.textContent=a.reason;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),1800);return}const c=current.course;UniStore.addPrint({screenId,courseCode:c.code,title:c.title});sessionStorage.setItem('unipop_print_payload',JSON.stringify({course:c,image:current.image||await UniImageStore.get(c.id)||'',displayText:current.displayText,qrUrl:currentQr}));t.textContent='Votre flyer est en cours d’impression…';t.classList.add('show');setTimeout(()=>t.classList.remove('show'),1800);window.open('flyer.html','unipop_flyer','width=900,height=1000')}
